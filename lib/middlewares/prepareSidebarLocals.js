 ...
 *         iconClass: Optional (String) End of bootstrap icon like 'table' in 'bi bi-table',
 *       },
 *
 *     ],
 *   },
      ...
      categories: [
        {
          name: 'Settings',
          items: [
            { name: 'Dashboard', url: '/#', isActive: false, iconClass: 'sliders' },
            ...Object.values(res.locals.models)
              .filter((m) => modelsInfo[m.tableName].isSdModel)
              // .filter((m) => !modelsInfo[m.tableName].isJoinTable && m.tableName !== 'sd_setting')
        ...
        {
          name: 'App Models',
          items: Object.values(res.locals.models)
            .filter((m) => !modelsInfo[m.tableName].isSdModel)
            .filter((m) => !modelsInfo[m.tableName]?.isJoinTable)
            .map((m) => {
              return {
